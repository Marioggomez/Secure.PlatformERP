Imports System.Data
Imports DevExpress.XtraGrid.Views.Grid
Imports Secure.Platform.Contracts.Dtos.Tercero
Imports Secure.Platform.WinForms.Forms.Base
Imports Secure.Platform.WinForms.Infrastructure

Namespace Forms.Terceros
    ''' <summary>
    ''' Pantalla de busqueda para tercero.direccion_tercero.
    ''' Autor: Mario Gomez.
    ''' </summary>
    Public Class FrmDireccionTerceroBuscar
        Inherits BaseSearchForm

        Public Sub New()
            Me.New(Nothing, Nothing)
        End Sub

        Public Sub New(ByVal apiClient As ApiClient, ByVal sessionContext As UserSessionContext)
            MyBase.New(apiClient, sessionContext)
        End Sub

        Protected Overrides Function BuildEndpoint() As String
            Return "api/v1/tercero/direccion_tercero/listar"
        End Function

        Protected Overrides Function BuildFormTitle() As String
            Return "Direcciones de Tercero - Busqueda"
        End Function

        Protected Overrides Sub ConfigureColumns(ByVal gridView As GridView)
            gridView.Columns.Clear()
            gridView.Columns.AddVisible("id_direccion_tercero", "Id").Width = 80
            gridView.Columns.AddVisible("id_tercero", "Id tercero").Width = 100
            gridView.Columns.AddVisible("id_tipo_direccion", "Tipo").Width = 90
            gridView.Columns.AddVisible("direccion_linea1", "Direccion 1").Width = 220
            gridView.Columns.AddVisible("direccion_linea2", "Direccion 2").Width = 220
            gridView.Columns.AddVisible("codigo_postal", "Postal").Width = 90
            gridView.Columns.AddVisible("principal", "Principal").Width = 90
        End Sub

        Protected Overrides Function BuildSampleData() As DataTable
            Dim table = CreateTable()

            Try
                If ApiClient IsNot Nothing Then
                    Dim items = ApiClient.GetAsync(Of List(Of DireccionTerceroDto))(BuildEndpoint()).GetAwaiter().GetResult()
                    If items IsNot Nothing Then
                        For Each item In items
                            table.Rows.Add(
                                item.IdDireccionTercero.GetValueOrDefault(0),
                                item.IdTercero.GetValueOrDefault(0),
                                item.IdTipoDireccion.GetValueOrDefault(0),
                                item.DireccionLinea1,
                                item.DireccionLinea2,
                                item.CodigoPostal,
                                item.Principal.GetValueOrDefault(False))
                        Next
                        Return table
                    End If
                End If
            Catch
                ' Fallback a data local de diseno.
            End Try

            Return table
        End Function

        Private Shared Function CreateTable() As DataTable
            Dim table As New DataTable()
            table.Columns.Add("id_direccion_tercero", GetType(Long))
            table.Columns.Add("id_tercero", GetType(Long))
            table.Columns.Add("id_tipo_direccion", GetType(Integer))
            table.Columns.Add("direccion_linea1", GetType(String))
            table.Columns.Add("direccion_linea2", GetType(String))
            table.Columns.Add("codigo_postal", GetType(String))
            table.Columns.Add("principal", GetType(Boolean))
            Return table
        End Function
    End Class
End Namespace

