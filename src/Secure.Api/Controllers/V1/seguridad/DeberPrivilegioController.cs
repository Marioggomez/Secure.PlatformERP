...
/// <summary>
/// Controller API v1 para la tabla seguridad.deber_privilegio.
/// Autor: Mario Gomez.
/// </summary>
[ApiController]
[Route("api/v1/seguridad/deber_privilegio")]
public sealed class DeberPrivilegioController : ControllerBase
{
    private readonly IDeberPrivilegioRepository _repository;

    public DeberPrivilegioController(IDeberPrivilegioRepository repository)
    {
    ...
    public async Task<ActionResult<object>> CrearAsync([FromBody] DeberPrivilegioDto dto, CancellationToken cancellationToken)
    {
        ...
        return CreatedAtAction(nameof(ObtenerAsync), new { idDeber = id }, new { id });
    }
