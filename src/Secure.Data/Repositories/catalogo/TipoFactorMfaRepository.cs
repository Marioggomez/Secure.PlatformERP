...
/// <summary>
/// Repositorio ADO.NET para catalogo.tipo_factor_mfa con stored procedures.
/// Autor: Mario Gomez.
/// </summary>
public sealed class TipoFactorMfaRepository : ITipoFactorMfaRepository
{
    private const string SpListar = "catalogo.usp_tipo_factor_mfa_listar";
    private const string SpObtener = "catalogo.usp_tipo_factor_mfa_obtener";
    private const string SpCrear = "catalogo.usp_tipo_factor_mfa_crear";
    private const string SpActualizar = "catalogo.usp_tipo_factor_mfa_actualizar";
    private const string SpDesactivar = "catalogo.usp_tipo_factor_mfa_desactivar";

    private readonly IDbConnectionFactory _connectionFactory;

    public TipoFactorMfaRepository(IDbConnectionFactory connectionFactory)
    {
        _connectionFactory = connectionFactory;
    }

    public async Task<IReadOnlyList<TipoFactorMfaDto>> ListarAsync(CancellationToken cancellationToken)
    {
        var result = new List<TipoFactorMfaDto>();
        ...
        {
            result.Add(new TipoFactorMfaDto
            {
    ...
    public async Task<TipoFactorMfaDto?> ObtenerAsync(short idTipoFactorMfa, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
        ...
        command.Parameters.Add(CreateParameter("@id_tipo_factor_mfa", SqlDbType.SmallInt, idTipoFactorMfa));
        using var reader = await command.ExecuteReaderAsync(cancellationToken).ConfigureAwait(false);
        if (await reader.ReadAsync(cancellationToken).ConfigureAwait(false))
        {
            return new TipoFactorMfaDto
            {
            IdTipoFactorMfa = reader.GetInt16(reader.GetOrdinal("id_tipo_factor_mfa")),
            Codigo = reader.IsDBNull(reader.GetOrdinal("codigo")) ? string.Empty : reader.GetString(reader.GetOrdinal("codigo")),
            Nombre = reader.IsDBNull(reader.GetOrdinal("nombre")) ? string.Empty : reader.GetString(reader.GetOrdinal("nombre")),
    ...
    public async Task<short> CrearAsync(TipoFactorMfaDto dto, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<bool> ActualizarAsync(TipoFactorMfaDto dto, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<bool> DesactivarAsync(short idTipoFactorMfa, string? usuario, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
