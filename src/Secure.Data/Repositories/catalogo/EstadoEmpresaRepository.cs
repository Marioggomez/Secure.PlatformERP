...
/// <summary>
/// Repositorio ADO.NET para catalogo.estado_empresa con stored procedures.
/// Autor: Mario Gomez.
/// </summary>
public sealed class EstadoEmpresaRepository : IEstadoEmpresaRepository
{
    private const string SpListar = "catalogo.usp_estado_empresa_listar";
    private const string SpObtener = "catalogo.usp_estado_empresa_obtener";
    private const string SpCrear = "catalogo.usp_estado_empresa_crear";
    private const string SpActualizar = "catalogo.usp_estado_empresa_actualizar";
    private const string SpDesactivar = "catalogo.usp_estado_empresa_desactivar";

    private readonly IDbConnectionFactory _connectionFactory;

    public EstadoEmpresaRepository(IDbConnectionFactory connectionFactory)
    {
        _connectionFactory = connectionFactory;
    }

    public async Task<IReadOnlyList<EstadoEmpresaDto>> ListarAsync(CancellationToken cancellationToken)
    {
        var result = new List<EstadoEmpresaDto>();
        ...
        {
            result.Add(new EstadoEmpresaDto
            {
    ...
    public async Task<EstadoEmpresaDto?> ObtenerAsync(short idEstadoEmpresa, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
        ...
        command.Parameters.Add(CreateParameter("@id_estado_empresa", SqlDbType.SmallInt, idEstadoEmpresa));
        using var reader = await command.ExecuteReaderAsync(cancellationToken).ConfigureAwait(false);
        if (await reader.ReadAsync(cancellationToken).ConfigureAwait(false))
        {
            return new EstadoEmpresaDto
            {
            IdEstadoEmpresa = reader.GetInt16(reader.GetOrdinal("id_estado_empresa")),
            Codigo = reader.IsDBNull(reader.GetOrdinal("codigo")) ? string.Empty : reader.GetString(reader.GetOrdinal("codigo")),
            Nombre = reader.IsDBNull(reader.GetOrdinal("nombre")) ? string.Empty : reader.GetString(reader.GetOrdinal("nombre")),
    ...
    public async Task<short> CrearAsync(EstadoEmpresaDto dto, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<bool> ActualizarAsync(EstadoEmpresaDto dto, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<bool> DesactivarAsync(short idEstadoEmpresa, string? usuario, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
