...
/// <summary>
/// Repositorio ADO.NET para catalogo.tipo_relacion_empresa con stored procedures.
/// Autor: Mario Gomez.
/// </summary>
public sealed class TipoRelacionEmpresaRepository : ITipoRelacionEmpresaRepository
{
    private const string SpListar = "catalogo.usp_tipo_relacion_empresa_listar";
    private const string SpObtener = "catalogo.usp_tipo_relacion_empresa_obtener";
    private const string SpCrear = "catalogo.usp_tipo_relacion_empresa_crear";
    private const string SpActualizar = "catalogo.usp_tipo_relacion_empresa_actualizar";
    private const string SpDesactivar = "catalogo.usp_tipo_relacion_empresa_desactivar";

    private readonly IDbConnectionFactory _connectionFactory;

    public TipoRelacionEmpresaRepository(IDbConnectionFactory connectionFactory)
    {
        _connectionFactory = connectionFactory;
    }

    public async Task<IReadOnlyList<TipoRelacionEmpresaDto>> ListarAsync(CancellationToken cancellationToken)
    {
        var result = new List<TipoRelacionEmpresaDto>();
        ...
        {
            result.Add(new TipoRelacionEmpresaDto
            {
    ...
    public async Task<TipoRelacionEmpresaDto?> ObtenerAsync(short idTipoRelacionEmpresa, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
        ...
        command.Parameters.Add(CreateParameter("@id_tipo_relacion_empresa", SqlDbType.SmallInt, idTipoRelacionEmpresa));
        using var reader = await command.ExecuteReaderAsync(cancellationToken).ConfigureAwait(false);
        if (await reader.ReadAsync(cancellationToken).ConfigureAwait(false))
        {
            return new TipoRelacionEmpresaDto
            {
            IdTipoRelacionEmpresa = reader.GetInt16(reader.GetOrdinal("id_tipo_relacion_empresa")),
            Codigo = reader.IsDBNull(reader.GetOrdinal("codigo")) ? string.Empty : reader.GetString(reader.GetOrdinal("codigo")),
            Nombre = reader.IsDBNull(reader.GetOrdinal("nombre")) ? string.Empty : reader.GetString(reader.GetOrdinal("nombre")),
    ...
    public async Task<short> CrearAsync(TipoRelacionEmpresaDto dto, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<bool> ActualizarAsync(TipoRelacionEmpresaDto dto, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<bool> DesactivarAsync(short idTipoRelacionEmpresa, string? usuario, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
