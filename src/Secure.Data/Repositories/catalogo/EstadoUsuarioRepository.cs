...
/// <summary>
/// Repositorio ADO.NET para catalogo.estado_usuario con stored procedures.
/// Autor: Mario Gomez.
/// </summary>
public sealed class EstadoUsuarioRepository : IEstadoUsuarioRepository
{
    private const string SpListar = "catalogo.usp_estado_usuario_listar";
    private const string SpObtener = "catalogo.usp_estado_usuario_obtener";
    private const string SpCrear = "catalogo.usp_estado_usuario_crear";
    private const string SpActualizar = "catalogo.usp_estado_usuario_actualizar";
    private const string SpDesactivar = "catalogo.usp_estado_usuario_desactivar";

    private readonly IDbConnectionFactory _connectionFactory;

    public EstadoUsuarioRepository(IDbConnectionFactory connectionFactory)
    {
        _connectionFactory = connectionFactory;
    }

    public async Task<IReadOnlyList<EstadoUsuarioDto>> ListarAsync(CancellationToken cancellationToken)
    {
        var result = new List<EstadoUsuarioDto>();
        ...
        {
            result.Add(new EstadoUsuarioDto
            {
    ...
    public async Task<EstadoUsuarioDto?> ObtenerAsync(short idEstadoUsuario, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
        ...
        command.Parameters.Add(CreateParameter("@id_estado_usuario", SqlDbType.SmallInt, idEstadoUsuario));
        using var reader = await command.ExecuteReaderAsync(cancellationToken).ConfigureAwait(false);
    ...
    public async Task<short> CrearAsync(EstadoUsuarioDto dto, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<bool> ActualizarAsync(EstadoUsuarioDto dto, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<bool> DesactivarAsync(short idEstadoUsuario, string? usuario, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
