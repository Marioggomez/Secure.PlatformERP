...
/// <summary>
/// Repositorio ADO.NET para catalogo.tipo_recurso_ui con stored procedures.
/// Autor: Mario Gomez.
/// </summary>
public sealed class TipoRecursoUiRepository : ITipoRecursoUiRepository
{
    private const string SpListar = "catalogo.usp_tipo_recurso_ui_listar";
    private const string SpObtener = "catalogo.usp_tipo_recurso_ui_obtener";
    private const string SpCrear = "catalogo.usp_tipo_recurso_ui_crear";
    private const string SpActualizar = "catalogo.usp_tipo_recurso_ui_actualizar";
    private const string SpDesactivar = "catalogo.usp_tipo_recurso_ui_desactivar";

    private readonly IDbConnectionFactory _connectionFactory;

    public TipoRecursoUiRepository(IDbConnectionFactory connectionFactory)
    {
        _connectionFactory = connectionFactory;
    }

    public async Task<IReadOnlyList<TipoRecursoUiDto>> ListarAsync(CancellationToken cancellationToken)
    {
        var result = new List<TipoRecursoUiDto>();
        ...
        {
            result.Add(new TipoRecursoUiDto
            {
    ...
    public async Task<TipoRecursoUiDto?> ObtenerAsync(short idTipoRecursoUi, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
        ...
        command.Parameters.Add(CreateParameter("@id_tipo_recurso_ui", SqlDbType.SmallInt, idTipoRecursoUi));
        using var reader = await command.ExecuteReaderAsync(cancellationToken).ConfigureAwait(false);
        if (await reader.ReadAsync(cancellationToken).ConfigureAwait(false))
        {
            return new TipoRecursoUiDto
            {
            IdTipoRecursoUi = reader.GetInt16(reader.GetOrdinal("id_tipo_recurso_ui")),
            Codigo = reader.IsDBNull(reader.GetOrdinal("codigo")) ? string.Empty : reader.GetString(reader.GetOrdinal("codigo")),
            Nombre = reader.IsDBNull(reader.GetOrdinal("nombre")) ? string.Empty : reader.GetString(reader.GetOrdinal("nombre")),
    ...
    public async Task<short> CrearAsync(TipoRecursoUiDto dto, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<bool> ActualizarAsync(TipoRecursoUiDto dto, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<bool> DesactivarAsync(short idTipoRecursoUi, string? usuario, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
