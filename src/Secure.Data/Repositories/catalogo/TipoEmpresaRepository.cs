...
/// <summary>
/// Repositorio ADO.NET para catalogo.tipo_empresa con stored procedures.
/// Autor: Mario Gomez.
/// </summary>
public sealed class TipoEmpresaRepository : ITipoEmpresaRepository
{
    private const string SpListar = "catalogo.usp_tipo_empresa_listar";
    private const string SpObtener = "catalogo.usp_tipo_empresa_obtener";
    private const string SpCrear = "catalogo.usp_tipo_empresa_crear";
    private const string SpActualizar = "catalogo.usp_tipo_empresa_actualizar";
    private const string SpDesactivar = "catalogo.usp_tipo_empresa_desactivar";

    private readonly IDbConnectionFactory _connectionFactory;

    public TipoEmpresaRepository(IDbConnectionFactory connectionFactory)
    {
        _connectionFactory = connectionFactory;
    }

    public async Task<IReadOnlyList<TipoEmpresaDto>> ListarAsync(CancellationToken cancellationToken)
    {
        var result = new List<TipoEmpresaDto>();
    ...
    public async Task<TipoEmpresaDto?> ObtenerAsync(short idTipoEmpresa, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
        ...
        command.Parameters.Add(CreateParameter("@id_tipo_empresa", SqlDbType.SmallInt, idTipoEmpresa));
        using var reader = await command.ExecuteReaderAsync(cancellationToken).ConfigureAwait(false);
    ...
    public async Task<short> CrearAsync(TipoEmpresaDto dto, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<bool> ActualizarAsync(TipoEmpresaDto dto, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<bool> DesactivarAsync(short idTipoEmpresa, string? usuario, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
