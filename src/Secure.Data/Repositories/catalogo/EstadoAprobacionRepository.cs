...
/// <summary>
/// Repositorio ADO.NET para catalogo.estado_aprobacion con stored procedures.
/// Autor: Mario Gomez.
/// </summary>
public sealed class EstadoAprobacionRepository : IEstadoAprobacionRepository
{
    private const string SpListar = "catalogo.usp_estado_aprobacion_listar";
    private const string SpObtener = "catalogo.usp_estado_aprobacion_obtener";
    private const string SpCrear = "catalogo.usp_estado_aprobacion_crear";
    private const string SpActualizar = "catalogo.usp_estado_aprobacion_actualizar";
    private const string SpDesactivar = "catalogo.usp_estado_aprobacion_desactivar";

    private readonly IDbConnectionFactory _connectionFactory;

    public EstadoAprobacionRepository(IDbConnectionFactory connectionFactory)
    {
        _connectionFactory = connectionFactory;
    }

    public async Task<IReadOnlyList<EstadoAprobacionDto>> ListarAsync(CancellationToken cancellationToken)
    {
        var result = new List<EstadoAprobacionDto>();
        ...
        {
            result.Add(new EstadoAprobacionDto
            {
    ...
    public async Task<EstadoAprobacionDto?> ObtenerAsync(short idEstadoAprobacion, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
        ...
        command.Parameters.Add(CreateParameter("@id_estado_aprobacion", SqlDbType.SmallInt, idEstadoAprobacion));
        using var reader = await command.ExecuteReaderAsync(cancellationToken).ConfigureAwait(false);
    ...
    public async Task<short> CrearAsync(EstadoAprobacionDto dto, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<bool> ActualizarAsync(EstadoAprobacionDto dto, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<bool> DesactivarAsync(short idEstadoAprobacion, string? usuario, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
