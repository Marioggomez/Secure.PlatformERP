...
/// <summary>
/// Repositorio ADO.NET para catalogo.tipo_verificacion_restablecimiento con stored procedures.
/// Autor: Mario Gomez.
/// </summary>
public sealed class TipoVerificacionRestablecimientoRepository : ITipoVerificacionRestablecimientoRepository
{
    private const string SpListar = "catalogo.usp_tipo_verificacion_restablecimiento_listar";
    private const string SpObtener = "catalogo.usp_tipo_verificacion_restablecimiento_obtener";
    private const string SpCrear = "catalogo.usp_tipo_verificacion_restablecimiento_crear";
    private const string SpActualizar = "catalogo.usp_tipo_verificacion_restablecimiento_actualizar";
    private const string SpDesactivar = "catalogo.usp_tipo_verificacion_restablecimiento_desactivar";

    private readonly IDbConnectionFactory _connectionFactory;

    public TipoVerificacionRestablecimientoRepository(IDbConnectionFactory connectionFactory)
    {
        _connectionFactory = connectionFactory;
    }

    public async Task<IReadOnlyList<TipoVerificacionRestablecimientoDto>> ListarAsync(CancellationToken cancellationToken)
    {
        var result = new List<TipoVerificacionRestablecimientoDto>();
        ...
        {
            result.Add(new TipoVerificacionRestablecimientoDto
            {
    ...
    public async Task<TipoVerificacionRestablecimientoDto?> ObtenerAsync(short idTipoVerificacionRestablecimiento, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
        ...
        {
            return new TipoVerificacionRestablecimientoDto
            {
            IdTipoVerificacionRestablecimiento = reader.GetInt16(reader.GetOrdinal("id_tipo_verificacion_restablecimiento")),
            Codigo = reader.IsDBNull(reader.GetOrdinal("codigo")) ? string.Empty : reader.GetString(reader.GetOrdinal("codigo")),
            Nombre = reader.IsDBNull(reader.GetOrdinal("nombre")) ? string.Empty : reader.GetString(reader.GetOrdinal("nombre")),
    ...
    public async Task<short> CrearAsync(TipoVerificacionRestablecimientoDto dto, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<bool> ActualizarAsync(TipoVerificacionRestablecimientoDto dto, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<bool> DesactivarAsync(short idTipoVerificacionRestablecimiento, string? usuario, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
