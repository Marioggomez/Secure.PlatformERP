...
/// <summary>
/// Repositorio ADO.NET para catalogo.accion_aprobacion con stored procedures.
/// Autor: Mario Gomez.
/// </summary>
public sealed class AccionAprobacionRepository : IAccionAprobacionRepository
{
    private const string SpListar = "catalogo.usp_accion_aprobacion_listar";
    private const string SpObtener = "catalogo.usp_accion_aprobacion_obtener";
    private const string SpCrear = "catalogo.usp_accion_aprobacion_crear";
    private const string SpActualizar = "catalogo.usp_accion_aprobacion_actualizar";
    private const string SpDesactivar = "catalogo.usp_accion_aprobacion_desactivar";

    private readonly IDbConnectionFactory _connectionFactory;

    public AccionAprobacionRepository(IDbConnectionFactory connectionFactory)
    {
        _connectionFactory = connectionFactory;
    }

    public async Task<IReadOnlyList<AccionAprobacionDto>> ListarAsync(CancellationToken cancellationToken)
    {
        var result = new List<AccionAprobacionDto>();
        ...
        {
            result.Add(new AccionAprobacionDto
            {
    ...
    public async Task<AccionAprobacionDto?> ObtenerAsync(short idAccionAprobacion, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
        ...
        command.Parameters.Add(CreateParameter("@id_accion_aprobacion", SqlDbType.SmallInt, idAccionAprobacion));
        using var reader = await command.ExecuteReaderAsync(cancellationToken).ConfigureAwait(false);
        if (await reader.ReadAsync(cancellationToken).ConfigureAwait(false))
        {
            return new AccionAprobacionDto
            {
            IdAccionAprobacion = reader.GetInt16(reader.GetOrdinal("id_accion_aprobacion")),
            Codigo = reader.IsDBNull(reader.GetOrdinal("codigo")) ? string.Empty : reader.GetString(reader.GetOrdinal("codigo")),
            Nombre = reader.IsDBNull(reader.GetOrdinal("nombre")) ? string.Empty : reader.GetString(reader.GetOrdinal("nombre")),
    ...
    public async Task<short> CrearAsync(AccionAprobacionDto dto, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<bool> ActualizarAsync(AccionAprobacionDto dto, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<bool> DesactivarAsync(short idAccionAprobacion, string? usuario, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
