...
/// <summary>
/// Repositorio ADO.NET para catalogo.proposito_desafio_mfa con stored procedures.
/// Autor: Mario Gomez.
/// </summary>
public sealed class PropositoDesafioMfaRepository : IPropositoDesafioMfaRepository
{
    private const string SpListar = "catalogo.usp_proposito_desafio_mfa_listar";
    private const string SpObtener = "catalogo.usp_proposito_desafio_mfa_obtener";
    private const string SpCrear = "catalogo.usp_proposito_desafio_mfa_crear";
    private const string SpActualizar = "catalogo.usp_proposito_desafio_mfa_actualizar";
    private const string SpDesactivar = "catalogo.usp_proposito_desafio_mfa_desactivar";

    private readonly IDbConnectionFactory _connectionFactory;

    public PropositoDesafioMfaRepository(IDbConnectionFactory connectionFactory)
    {
        _connectionFactory = connectionFactory;
    }

    public async Task<IReadOnlyList<PropositoDesafioMfaDto>> ListarAsync(CancellationToken cancellationToken)
    {
        var result = new List<PropositoDesafioMfaDto>();
        ...
        {
            result.Add(new PropositoDesafioMfaDto
            {
    ...
    public async Task<PropositoDesafioMfaDto?> ObtenerAsync(short idPropositoDesafioMfa, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
        ...
        {
            return new PropositoDesafioMfaDto
            {
            IdPropositoDesafioMfa = reader.GetInt16(reader.GetOrdinal("id_proposito_desafio_mfa")),
            Codigo = reader.IsDBNull(reader.GetOrdinal("codigo")) ? string.Empty : reader.GetString(reader.GetOrdinal("codigo")),
            Nombre = reader.IsDBNull(reader.GetOrdinal("nombre")) ? string.Empty : reader.GetString(reader.GetOrdinal("nombre")),
    ...
    public async Task<short> CrearAsync(PropositoDesafioMfaDto dto, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<bool> ActualizarAsync(PropositoDesafioMfaDto dto, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<bool> DesactivarAsync(short idPropositoDesafioMfa, string? usuario, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
