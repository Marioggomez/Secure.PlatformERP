...
/// <summary>
/// Repositorio ADO.NET para catalogo.canal_notificacion con stored procedures.
/// Autor: Mario Gomez.
/// </summary>
public sealed class CanalNotificacionRepository : ICanalNotificacionRepository
{
    private const string SpListar = "catalogo.usp_canal_notificacion_listar";
    private const string SpObtener = "catalogo.usp_canal_notificacion_obtener";
    private const string SpCrear = "catalogo.usp_canal_notificacion_crear";
    private const string SpActualizar = "catalogo.usp_canal_notificacion_actualizar";
    private const string SpDesactivar = "catalogo.usp_canal_notificacion_desactivar";

    private readonly IDbConnectionFactory _connectionFactory;

    public CanalNotificacionRepository(IDbConnectionFactory connectionFactory)
    {
        _connectionFactory = connectionFactory;
    }

    public async Task<IReadOnlyList<CanalNotificacionDto>> ListarAsync(CancellationToken cancellationToken)
    {
        var result = new List<CanalNotificacionDto>();
        ...
        {
            result.Add(new CanalNotificacionDto
            {
    ...
    public async Task<CanalNotificacionDto?> ObtenerAsync(short idCanalNotificacion, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<short> CrearAsync(CanalNotificacionDto dto, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<bool> ActualizarAsync(CanalNotificacionDto dto, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<bool> DesactivarAsync(short idCanalNotificacion, string? usuario, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
