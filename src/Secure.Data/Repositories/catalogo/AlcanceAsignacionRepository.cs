...
/// <summary>
/// Repositorio ADO.NET para catalogo.alcance_asignacion con stored procedures.
/// Autor: Mario Gomez.
/// </summary>
public sealed class AlcanceAsignacionRepository : IAlcanceAsignacionRepository
{
    private const string SpListar = "catalogo.usp_alcance_asignacion_listar";
    private const string SpObtener = "catalogo.usp_alcance_asignacion_obtener";
    private const string SpCrear = "catalogo.usp_alcance_asignacion_crear";
    private const string SpActualizar = "catalogo.usp_alcance_asignacion_actualizar";
    private const string SpDesactivar = "catalogo.usp_alcance_asignacion_desactivar";

    private readonly IDbConnectionFactory _connectionFactory;

    public AlcanceAsignacionRepository(IDbConnectionFactory connectionFactory)
    {
        _connectionFactory = connectionFactory;
    }

    public async Task<IReadOnlyList<AlcanceAsignacionDto>> ListarAsync(CancellationToken cancellationToken)
    {
        var result = new List<AlcanceAsignacionDto>();
        ...
        {
            result.Add(new AlcanceAsignacionDto
            {
    ...
    public async Task<AlcanceAsignacionDto?> ObtenerAsync(short idAlcanceAsignacion, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
        ...
        command.Parameters.Add(CreateParameter("@id_alcance_asignacion", SqlDbType.SmallInt, idAlcanceAsignacion));
        using var reader = await command.ExecuteReaderAsync(cancellationToken).ConfigureAwait(false);
        if (await reader.ReadAsync(cancellationToken).ConfigureAwait(false))
        {
            return new AlcanceAsignacionDto
            {
            IdAlcanceAsignacion = reader.GetInt16(reader.GetOrdinal("id_alcance_asignacion")),
            Codigo = reader.IsDBNull(reader.GetOrdinal("codigo")) ? string.Empty : reader.GetString(reader.GetOrdinal("codigo")),
            Nombre = reader.IsDBNull(reader.GetOrdinal("nombre")) ? string.Empty : reader.GetString(reader.GetOrdinal("nombre")),
    ...
    public async Task<short> CrearAsync(AlcanceAsignacionDto dto, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<bool> ActualizarAsync(AlcanceAsignacionDto dto, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<bool> DesactivarAsync(short idAlcanceAsignacion, string? usuario, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
