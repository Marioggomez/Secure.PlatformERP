...
/// <summary>
/// Repositorio ADO.NET para catalogo.tipo_evento_seguridad con stored procedures.
/// Autor: Mario Gomez.
/// </summary>
public sealed class TipoEventoSeguridadRepository : ITipoEventoSeguridadRepository
{
    private const string SpListar = "catalogo.usp_tipo_evento_seguridad_listar";
    private const string SpObtener = "catalogo.usp_tipo_evento_seguridad_obtener";
    private const string SpCrear = "catalogo.usp_tipo_evento_seguridad_crear";
    private const string SpActualizar = "catalogo.usp_tipo_evento_seguridad_actualizar";
    private const string SpDesactivar = "catalogo.usp_tipo_evento_seguridad_desactivar";

    private readonly IDbConnectionFactory _connectionFactory;

    public TipoEventoSeguridadRepository(IDbConnectionFactory connectionFactory)
    {
        _connectionFactory = connectionFactory;
    }

    public async Task<IReadOnlyList<TipoEventoSeguridadDto>> ListarAsync(CancellationToken cancellationToken)
    {
        var result = new List<TipoEventoSeguridadDto>();
        ...
        {
            result.Add(new TipoEventoSeguridadDto
            {
    ...
    public async Task<TipoEventoSeguridadDto?> ObtenerAsync(short idTipoEventoSeguridad, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
        ...
        command.Parameters.Add(CreateParameter("@id_tipo_evento_seguridad", SqlDbType.SmallInt, idTipoEventoSeguridad));
        using var reader = await command.ExecuteReaderAsync(cancellationToken).ConfigureAwait(false);
        if (await reader.ReadAsync(cancellationToken).ConfigureAwait(false))
        {
            return new TipoEventoSeguridadDto
            {
            IdTipoEventoSeguridad = reader.GetInt16(reader.GetOrdinal("id_tipo_evento_seguridad")),
            Codigo = reader.IsDBNull(reader.GetOrdinal("codigo")) ? string.Empty : reader.GetString(reader.GetOrdinal("codigo")),
            Nombre = reader.IsDBNull(reader.GetOrdinal("nombre")) ? string.Empty : reader.GetString(reader.GetOrdinal("nombre")),
    ...
    public async Task<short> CrearAsync(TipoEventoSeguridadDto dto, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<bool> ActualizarAsync(TipoEventoSeguridadDto dto, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<bool> DesactivarAsync(short idTipoEventoSeguridad, string? usuario, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
