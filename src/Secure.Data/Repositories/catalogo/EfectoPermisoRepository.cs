...
/// <summary>
/// Repositorio ADO.NET para catalogo.efecto_permiso con stored procedures.
/// Autor: Mario Gomez.
/// </summary>
public sealed class EfectoPermisoRepository : IEfectoPermisoRepository
{
    private const string SpListar = "catalogo.usp_efecto_permiso_listar";
    private const string SpObtener = "catalogo.usp_efecto_permiso_obtener";
    private const string SpCrear = "catalogo.usp_efecto_permiso_crear";
    private const string SpActualizar = "catalogo.usp_efecto_permiso_actualizar";
    private const string SpDesactivar = "catalogo.usp_efecto_permiso_desactivar";

    private readonly IDbConnectionFactory _connectionFactory;

    public EfectoPermisoRepository(IDbConnectionFactory connectionFactory)
    {
        _connectionFactory = connectionFactory;
    }

    public async Task<IReadOnlyList<EfectoPermisoDto>> ListarAsync(CancellationToken cancellationToken)
    {
        var result = new List<EfectoPermisoDto>();
        ...
        {
            result.Add(new EfectoPermisoDto
            {
    ...
    public async Task<EfectoPermisoDto?> ObtenerAsync(short idEfectoPermiso, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
        ...
        command.Parameters.Add(CreateParameter("@id_efecto_permiso", SqlDbType.SmallInt, idEfectoPermiso));
        using var reader = await command.ExecuteReaderAsync(cancellationToken).ConfigureAwait(false);
        if (await reader.ReadAsync(cancellationToken).ConfigureAwait(false))
        {
            return new EfectoPermisoDto
            {
            IdEfectoPermiso = reader.GetInt16(reader.GetOrdinal("id_efecto_permiso")),
            Codigo = reader.IsDBNull(reader.GetOrdinal("codigo")) ? string.Empty : reader.GetString(reader.GetOrdinal("codigo")),
            Nombre = reader.IsDBNull(reader.GetOrdinal("nombre")) ? string.Empty : reader.GetString(reader.GetOrdinal("nombre")),
    ...
    public async Task<short> CrearAsync(EfectoPermisoDto dto, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<bool> ActualizarAsync(EfectoPermisoDto dto, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<bool> DesactivarAsync(short idEfectoPermiso, string? usuario, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
