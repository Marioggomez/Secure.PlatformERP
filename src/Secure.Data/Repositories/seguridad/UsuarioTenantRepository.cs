...
/// <summary>
/// Repositorio ADO.NET para seguridad.usuario_tenant con stored procedures.
/// Autor: Mario Gomez.
/// </summary>
public sealed class UsuarioTenantRepository : IUsuarioTenantRepository
{
    private const string SpListar = "seguridad.usp_usuario_tenant_listar";
    private const string SpObtener = "seguridad.usp_usuario_tenant_obtener";
    private const string SpCrear = "seguridad.usp_usuario_tenant_crear";
    private const string SpActualizar = "seguridad.usp_usuario_tenant_actualizar";
    private const string SpDesactivar = "seguridad.usp_usuario_tenant_desactivar";

    private readonly IDbConnectionFactory _connectionFactory;

    public UsuarioTenantRepository(IDbConnectionFactory connectionFactory)
    {
        _connectionFactory = connectionFactory;
    }

    public async Task<IReadOnlyList<UsuarioTenantDto>> ListarAsync(CancellationToken cancellationToken)
    {
        var result = new List<UsuarioTenantDto>();
        ...
        {
            result.Add(new UsuarioTenantDto
            {
            IdUsuario = reader.GetInt64(reader.GetOrdinal("id_usuario")),
            IdTenant = reader.GetInt64(reader.GetOrdinal("id_tenant")),
            EsAdministradorTenant = reader.GetBoolean(reader.GetOrdinal("es_administrador_tenant")),
            EsCuentaServicio = reader.GetBoolean(reader.GetOrdinal("es_cuenta_servicio")),
    ...
    public async Task<UsuarioTenantDto?> ObtenerAsync(long idUsuario, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<long> CrearAsync(UsuarioTenantDto dto, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
