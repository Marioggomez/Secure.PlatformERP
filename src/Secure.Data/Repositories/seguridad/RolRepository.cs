...
/// <summary>
/// Repositorio ADO.NET para seguridad.rol con stored procedures.
/// Autor: Mario Gomez.
/// </summary>
public sealed class RolRepository : IRolRepository
{
    private const string SpListar = "seguridad.usp_rol_listar";
    private const string SpObtener = "seguridad.usp_rol_obtener";
    private const string SpCrear = "seguridad.usp_rol_crear";
    private const string SpActualizar = "seguridad.usp_rol_actualizar";
    private const string SpDesactivar = "seguridad.usp_rol_desactivar";
    ...
    public async Task<IReadOnlyList<RolDto>> ListarAsync(CancellationToken cancellationToken)
    {
    ...
    public async Task<RolDto?> ObtenerAsync(long idRol, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<long> CrearAsync(RolDto dto, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
    ...
    public async Task<bool> ActualizarAsync(RolDto dto, CancellationToken cancellationToken)
    {
        using var connection = _connectionFactory.CreateConnection();
        await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        using var command = connection.CreateCommand();
