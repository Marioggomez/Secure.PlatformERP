...
SET XACT_ABORT ON;
GO
/* Rollback de listados scoped a comportamiento legado sin filtro */
...
END
GO
/* Restaurar FKs legacy eliminadas */
IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name=N'FK_empresa_tenant' AND parent_object_id=OBJECT_ID(N'organizacion.empresa')) ALTER TABLE organizacion.empresa WITH CHECK ADD CONSTRAINT FK_empresa_tenant FOREIGN KEY(id_tenant) REFERENCES plataforma.tenant(id_tenant);
IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name=N'FK_usuario_tenant_tenant' AND parent_object_id=OBJECT_ID(N'seguridad.usuario_tenant')) ALTER TABLE seguridad.usuario_tenant WITH CHECK ADD CONSTRAINT FK_usuario_tenant_tenant FOREIGN KEY(id_tenant) REFERENCES plataforma.tenant(id_tenant);
IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name=N'FK_usuario_tenant_usuario' AND parent_object_id=OBJECT_ID(N'seguridad.usuario_tenant')) ALTER TABLE seguridad.usuario_tenant WITH CHECK ADD CONSTRAINT FK_usuario_tenant_usuario FOREIGN KEY(id_usuario) REFERENCES seguridad.usuario(id_usuario);
/* Rollback indices de performance */
IF EXISTS (SELECT 1 FROM sys.indexes WHERE name=N'IX_seguridad_sesion_usuario_scope' AND object_id=OBJECT_ID(N'seguridad.sesion_usuario')) DROP INDEX IX_seguridad_sesion_usuario_scope ON seguridad.sesion_usuario;
IF EXISTS (SELECT 1 FROM sys.indexes WHERE name=N'IX_seguridad_usuario_empresa_scope' AND object_id=OBJECT_ID(N'seguridad.usuario_empresa')) DROP INDEX IX_seguridad_usuario_empresa_scope ON seguridad.usuario_empresa;
IF EXISTS (SELECT 1 FROM sys.indexes WHERE name=N'IX_organizacion_unidad_organizativa_scope' AND object_id=OBJECT_ID(N'organizacion.unidad_organizativa')) DROP INDEX IX_organizacion_unidad_organizativa_scope ON organizacion.unidad_organizativa;
IF EXISTS (SELECT 1 FROM sys.indexes WHERE name=N'IX_cumplimiento_instancia_aprobacion_scope' AND object_id=OBJECT_ID(N'cumplimiento.instancia_aprobacion')) DROP INDEX IX_cumplimiento_instancia_aprobacion_scope ON cumplimiento.instancia_aprobacion;
IF EXISTS (SELECT 1 FROM sys.indexes WHERE name=N'IX_logistica_factor_volumetrico_scope' AND object_id=OBJECT_ID(N'logistica.factor_volumetrico')) DROP INDEX IX_logistica_factor_volumetrico_scope ON logistica.factor_volumetrico;
IF EXISTS (SELECT 1 FROM sys.indexes WHERE name=N'IX_logistica_ruta_logistica_scope' AND object_id=OBJECT_ID(N'logistica.ruta_logistica')) DROP INDEX IX_logistica_ruta_logistica_scope ON logistica.ruta_logistica;
IF EXISTS (SELECT 1 FROM sys.indexes WHERE name=N'IX_logistica_temporada_tarifaria_scope' AND object_id=OBJECT_ID(N'logistica.temporada_tarifaria')) DROP INDEX IX_logistica_temporada_tarifaria_scope ON logistica.temporada_tarifaria;
PRINT N'Rollback aplicado: hardening listados scoped + higiene FK + indices scope.';
